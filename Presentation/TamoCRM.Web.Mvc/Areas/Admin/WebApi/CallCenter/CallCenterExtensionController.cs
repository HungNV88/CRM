...
{
    public class CallCenterExtensionController : CustomApiController
    {
        public IEnumerable<CallCenterExtensionInfo> Get()
        {
            return CallCenterExtensionRepository.GetAll();
        }

        // GET api/<controller>
        public CallCenterExtensionListModel Get(int page, int rows)
        {
            int totalRecords;
            var model = new CallCenterExtensionListModel
            {
                Rows = CallCenterExtensionRepository.Search(string.Empty, page, rows, out totalRecords),
                Page = page,
